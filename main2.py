...
The PRIMARY comparison is always:
  STAKEHOLDER COMMENT  →  FINAL REGULATION

The draft is provided ONLY to establish what already existed BEFORE the comment.
The draft is NOT a source of evidence that a comment was implemented.

A comment is ACCEPTED only if the FINAL REGULATION contains something NEW or CHANGED
that directly responds to the suggestion made in the comment.

If the final regulation is identical to the draft on the issue raised by the comment,
the comment is REJECTED — regardless of what the draft says.

A clause that existed in the draft and is UNCHANGED in the final is NOT evidence
that any comment was implemented. It was already there. The regulator made no change.

═══════════════════════════════════════════════════════

Always follow this reasoning chain:

Draft Position  (what existed BEFORE — reference only, NOT evidence)
→ Requested Change  (what the stakeholder asked for)
→ Final Position  (what actually changed in the final)
→ Implementation Status

The question is:

"What did the stakeholder ask for, and did the FINAL REGULATION introduce something NEW or CHANGED that delivers that request?"
...
def classify_comment(
    ...
    user_content = f"""════════════════════════════════════════════════════════
STEP 1 — DRAFT CLAUSES (BASELINE ONLY — NOT EVIDENCE)
These show what the regulation said BEFORE the comment was submitted.
Use these ONLY to understand what the stakeholder is reacting to.
Do NOT use these as evidence that any comment was implemented.
════════════════════════════════════════════════════════
{draft_context if draft_context else "Not found in draft."}

════════════════════════════════════════════════════════
STEP 2 — STAKEHOLDER COMMENT (this is what you must evaluate)
This is the suggestion made by the stakeholder.
The non-italic text is the actual recommendation.
The italic text (in the original PDF) is quoted draft clause — context only.
Your job: identify what specific change(s) the stakeholder is requesting.
════════════════════════════════════════════════════════
{suggestion_full}

════════════════════════════════════════════════════════
STEP 3 — FINAL REGULATION EXCERPTS (PRIMARY EVIDENCE SOURCE)
Compare the STAKEHOLDER COMMENT above against these final clauses.
Each clause is labelled with its change status vs the draft:
  [UNCHANGED FROM DRAFT]  → identical to draft; CANNOT be evidence of implementation
  [MODIFIED FROM DRAFT]   → changed; only the changed portion may be evidence
  [NEW in final]          → did not exist in draft; valid evidence if it matches the comment
════════════════════════════════════════════════════════
...
Q1. What is the comment actually asking for?
    Read STEP 2 carefully — the stakeholder's suggestion (non-italic text).
    — Is it requesting a CHANGE or ADDITION to the draft?
    — Or is it only ENDORSING / SUPPORTING something already in the draft?
    If endorsing only → REJECTED (there is nothing new to implement).

Q2. For each specific request identified in Q1:
    Look at the FINAL EXCERPTS in STEP 3.
    Check the [UNCHANGED / MODIFIED / NEW] label on each final clause.

    ██ ABSOLUTE RULE ██
    A final clause labelled [UNCHANGED FROM DRAFT] existed BEFORE the comment.
    It CANNOT prove the comment was implemented — the regulator changed nothing.
    Citing an [UNCHANGED FROM DRAFT] clause as evidence is always WRONG.

Q3. Determine for each request:
    — Final clause is [NEW in final] AND directly delivers the request → IMPLEMENTED
    — Final clause is [MODIFIED FROM DRAFT] AND the modification delivers the request → IMPLEMENTED
    — Final clause is [UNCHANGED FROM DRAFT] → NOT IMPLEMENTED (pre-existed the comment)
    — No corresponding clause exists in final → NOT IMPLEMENTED

Q4. Classify:
    ACCEPTED           → ALL requests implemented via [NEW] or [MODIFIED] clauses only
    PARTIALLY_ACCEPTED → SOME requests implemented, SOME not (must have both; multiple distinct requests required)
    REJECTED           → NO request was implemented; OR comment only endorsed existing draft text

Q5. Evidence:
    — Quote ONLY clauses labelled [NEW in final] or the changed portion of [MODIFIED FROM DRAFT] clauses.
    — NEVER quote a clause labelled [UNCHANGED FROM DRAFT] as evidence.
    — NEVER quote draft text as evidence.
    — If no valid evidence exists → write exactly: "Not found in final regulation"
""".strip()
